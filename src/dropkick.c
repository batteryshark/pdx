...
void usage(char* exe_name) {
	printf("Usage: %s start PATH_TO_BOOTSTRAP PATH_TO_EXE [EXE_ARGS]\n", exe_name);
	printf("Usage: %s start_in PATH_TO_BOOTSTRAP PATH_TO_EXE START_PATH [EXE_ARGS]\n", exe_name);	
	printf("Usage: %s inject PATH_TO_BOOTSTRAP PID TID LEAVE_SUSPENDED\n",exe_name);
	exit(-1);
}
